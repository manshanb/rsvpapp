  ...
  stages {
    ...
    stage('Deploy to staging') {
      environment {
        ...
        GIT_REPO_URL = "github.com/manshanb/rsvpapp-kustomize.git"
      ...
      steps {
        container('tools') {
          ...
          dir("rsvpapp-kustomize") {
              ...
              sh "cd ./overlays/staging && kustomize edit set image manshanb/rsvpapp=${env.IMAGE_REPO}:${env.GIT_COMMIT}"
    ...
    stage('Deploy to Prod') {
      steps {
        ...
        container('tools') {
          dir("rsvpapp-kustomize") {
              sh "cd ./overlays/prod && kustomize edit set image manshanb/rsvpapp=${env.IMAGE_REPO}:${env.GIT_COMMIT}"
            sh "git commit -am 'Publish new version' && git push || echo 'no changes'"
          }
        }
      }
